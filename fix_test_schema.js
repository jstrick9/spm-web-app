...
describe('Staff RBAC Integration', () => {
  ...
  beforeEach(async () => {
    ...
    // Create an org and owner
`;

code = code.replace(/import \{ applyAllMigrations \} from '\.\.\/db\/migrate\.js';[\s\S]*?const owner = usersRepo\.create/m, "import { applyAllMigrations } from '../db/migrate.js';\n\ndescribe('Staff RBAC Integration', () => {\n  let app: FastifyInstance;\n  let orgId: string;\n  let adminToken: string;\n  let staffToken: string;\n  let staffId: string;\n\n  beforeEach(async () => {\n    app = await buildApp();\n    db.exec('BEGIN');\n\n    const owner = usersRepo.create");
