...
const regex = /\/\/ ─── Public Guest Portal \(unchanged from Phase 2 — Day 4 rebuilds\) ────[\s\S]*?function GuestPortal\(\{ eventId \}: \{ eventId: string \}\) \{[\s\S]*?\}\n/m;
...
import { Map as MapIcon, Home, Send, Tent } from 'lucide-react';
...
export function PublicGuestPortal({ eventId }: { eventId: string }) {
  ...
  if (!info) return <div className="min-h-screen bg-bg flex items-center justify-center"><div className="animate-pulse text-fg-muted">Loading Portal...</div></div>;
  ...
  return (
    <div className="min-h-screen bg-[#fdfbf7] font-serif text-[#2c3e2e] flex flex-col relative pb-20">
      <header className="bg-white border-b border-[#e1d5c9] py-6 px-4 text-center sticky top-0 z-10">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full p-4 md:p-8">
         ...
         {activeTab === 'map' && (
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-500 h-full flex flex-col">
               ...
               {activeGuest && layout && (
                  <div className="bg-[#e1d5c9]/30 p-4 rounded-lg flex items-center justify-center gap-2">
...
`;
  fs.writeFileSync('spm-web-app/wedding-app/client/src/screens/portal/PublicGuestPortal.tsx', portalCode);
} else {
