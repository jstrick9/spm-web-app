  ...
  return (
    <>
      ...
      <PageBody className="space-y-5 sm:space-y-7 bg-bg/30 min-h-[calc(100vh-10rem)] rounded-xl p-3 sm:p-6 border border-border/40">
        ...
        {isManager && (
          <>
          <Card><CardHeader><div className="flex items-center justify-between gap-2"><div><CardTitle>Staffing calendar</CardTitle><CardDescription>Venue-wide monthly commitments. Select an event to drill into its event-week staff assignments.</CardDescription></div><div className="flex items-center gap-2"><Button size="xs" variant="outline" onClick={() => setStaffingCalendarMonthOffset((value) => value - 1)}>Previous</Button><span className="text-sm font-medium">{new Date(staffingCalendarRange.start).toLocaleString(undefined, { month: 'long', year: 'numeric' })}</span><Button size="xs" variant="outline" onClick={() => setStaffingCalendarMonthOffset((value) => value + 1)}>Next</Button></div></div></CardHeader><CardContent>{staffingCalendarQuery.isLoading ? <p className="text-sm text-fg-muted">Loading calendar…</p> : <div className="grid grid-cols-7 gap-1 text-xs"><div className="col-span-7 grid grid-cols-7 gap-1 text-center font-semibold text-fg-muted">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((day) => <span key={day}>{day}</span>)}</div>{Array.from({ length: new Date(new Date(staffingCalendarRange.start).getFullYear(), new Date(staffingCalendarRange.start).getMonth(), 1).getDay() }).map((_, index) => <div key={`blank-${index}`} className="min-h-16 rounded border border-transparent" />)}{Array.from({ length: staffingCalendarRange.days }).map((_, index) => { const day = index + 1; const dateKey = `${new Date(staffingCalendarRange.start).getFullYear()}-${String(new Date(staffingCalendarRange.start).getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`; const events = staffingCalendarQuery.data?.calendar.events.filter((event) => event.start_date === dateKey) || []; const shifts = staffingCalendarQuery.data?.calendar.shifts.filter((shift: any) => String(shift.starts_at).slice(0, 10) === dateKey) || []; return <div key={dateKey} className="min-h-16 rounded border border-border p-1"><strong>{day}</strong>{events.map((event) => <button key={event.id} onClick={() => { window.location.hash = `#/events/${event.id}?tab=staff`; }} className="mt-1 block w-full truncate rounded bg-brand-soft px-1 text-left text-brand hover:underline">{event.title}</button>)}{shifts.length > 0 && <p className="mt-1 text-fg-muted">{shifts.length} shift{shifts.length === 1 ? '' : 's'}</p>}</div>; })}</div>}</CardContent></Card>
          <Card className="border-brand/20"><CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-brand" /> Staffing coverage</CardTitle><CardDescription>Cross-event coverage for Seven Paths Manor. Resolve overlaps before event week.</CardDescription></CardHeader><CardContent>{staffingCoverageQuery.isLoading ? <p className="text-sm text-fg-muted">Loading staffing coverage…</p> : <div className="space-y-2">{staffingCoverageQuery.data?.coverage.conflicts.length ? <p className="flex items-center gap-1 text-sm text-warning"><AlertTriangle className="h-4 w-4" /> {staffingCoverageQuery.data.coverage.conflicts.length} overlapping shift{staffingCoverageQuery.data.coverage.conflicts.length === 1 ? '' : 's'} need attention.</p> : <p className="text-sm text-success">No overlapping staff shifts detected.</p>}{staffingCoverageQuery.data?.coverage.events.map((coverage) => <div key={coverage.eventId || 'unassigned'} className="flex items-center justify-between gap-2 rounded border border-border p-2 text-sm"><span><strong>{coverage.eventTitle}</strong><span className="ml-2 text-fg-muted">{coverage.shifts.length} shift{coverage.shifts.length === 1 ? '' : 's'} · {coverage.taskCount} task{coverage.taskCount === 1 ? '' : 's'}{coverage.blockedTaskCount ? ` · ${coverage.blockedTaskCount} blocked` : ''}</span></span><span className="flex items-center gap-2 text-fg-muted">{coverage.staffCount} staff{coverage.eventId && <Button size="xs" variant="outline" onClick={() => { window.location.hash = `#/events/${coverage.eventId}?tab=staff`; }}>Manage</Button>}</span></div>)}{staffingCoverageQuery.data?.coverage.staff?.length ? <div className="mt-3 border-t border-border pt-3"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-fg-muted">Staff workload</p>{staffingCoverageQuery.data.coverage.staff.map((staff) => <div key={staff.staffId} className="flex justify-between py-1 text-sm"><span>{staff.staffName}</span><span className={staff.conflictCount ? 'text-warning' : 'text-fg-muted'}>{staff.shiftCount} shifts · {staff.eventCount} event{staff.eventCount === 1 ? '' : 's'}{staff.conflictCount ? ` · ${staff.conflictCount} conflict${staff.conflictCount === 1 ? '' : 's'}` : ''}</span></div>)}</div> : null}</div>}</CardContent></Card>
