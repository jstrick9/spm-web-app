...
export function DashboardScreen({
  ...
  const spaceCalendarRange = useMemo(() => {
    const now = new Date();
    const year = now.getFullYear();
    const month = now.getMonth() + spaceCalendarMonthOffset;
    const start = new Date(year, month, 1);
    const end = new Date(year, month + 1, 1);
    return { startDate: localDateString(start), endDate: localDateString(end), year: start.getFullYear(), month: start.getMonth() };
  }, [spaceCalendarMonthOffset]);
  const spaceCalendarQuery = useQuery({ queryKey: ['space-calendar', orgId, spaceCalendarRange.startDate], queryFn: () => sdk.venues.spaceCalendar(orgId!, spaceCalendarRange.startDate, spaceCalendarRange.endDate), enabled: !!orgId && isManager, staleTime: 30_000 });
