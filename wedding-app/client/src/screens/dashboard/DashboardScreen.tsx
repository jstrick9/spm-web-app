...
export function DashboardScreen({
  ...
  const spaceCalendarQuery = useQuery({ queryKey: ['space-calendar', orgId, staffingCalendarRange.start], queryFn: () => sdk.venues.spaceCalendar(orgId!, staffingCalendarRange.start.slice(0, 10), staffingCalendarRange.end.slice(0, 10)), enabled: !!orgId && isManager, staleTime: 30_000 });
  ...
  return (
    <>
      ...
      <PageBody className="space-y-5 sm:space-y-7 bg-bg/30 min-h-[calc(100vh-10rem)] rounded-xl p-3 sm:p-6 border border-border/40">
        ...
        {isManager && (
          <>
          <Card><CardHeader><CardTitle>Venue space calendar</CardTitle><CardDescription>Current-month space commitments and capacity context for active bookings.</CardDescription></CardHeader><CardContent>{spaceCalendarQuery.isLoading ? <p className="text-sm text-fg-muted">Loading space calendar…</p> : <div className="space-y-2">{spaceCalendarQuery.data?.calendar.commitments.length ? spaceCalendarQuery.data.calendar.commitments.map((commitment: any) => <div key={commitment.id} className="flex flex-wrap items-center justify-between gap-2 rounded border border-border p-2 text-sm"><div><strong>{commitment.venue_name || 'Space not assigned'}</strong><p className="text-fg-muted">{commitment.title} · {commitment.start_date} · {commitment.guest_count} guests{commitment.venue_capacity ? ` / ${commitment.venue_capacity} capacity` : ''}</p>{commitment.venue_capacity && commitment.guest_count > commitment.venue_capacity && <p className="mt-1 text-xs text-warning">Guest count exceeds this space’s capacity.</p>}</div><Button size="xs" variant="outline" onClick={() => { window.location.hash = `#/events/${commitment.id}`; }}>Open event</Button></div>) : <p className="text-sm text-fg-muted">No space commitments this month.</p>}</div>}</CardContent></Card>
