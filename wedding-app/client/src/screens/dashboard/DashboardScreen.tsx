  ...
  return (
    <>
      ...
      <PageBody className="space-y-5 sm:space-y-7 bg-bg/30 min-h-[calc(100vh-10rem)] rounded-xl p-3 sm:p-6 border border-border/40">
        ...
        {isManager && (
          <>
          <Card className="border-brand/20"><CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-brand" /> Staffing coverage</CardTitle><CardDescription>Cross-event coverage for Seven Paths Manor. Resolve overlaps before event week.</CardDescription></CardHeader><CardContent>{staffingCoverageQuery.isLoading ? <p className="text-sm text-fg-muted">Loading staffing coverage…</p> : <div className="space-y-2">{staffingCoverageQuery.data?.coverage.conflicts.length ? <p className="flex items-center gap-1 text-sm text-warning"><AlertTriangle className="h-4 w-4" /> {staffingCoverageQuery.data.coverage.conflicts.length} overlapping shift{staffingCoverageQuery.data.coverage.conflicts.length === 1 ? '' : 's'} need attention.</p> : <p className="text-sm text-success">No overlapping staff shifts detected.</p>}{staffingCoverageQuery.data?.coverage.staff?.length ? <div className="rounded border border-border bg-surface-2 p-2 text-sm"><strong>Team workload</strong><div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-fg-muted">{staffingCoverageQuery.data.coverage.staff.map((staff) => <span key={staff.staffId}>{staff.staffName}: {staff.shiftCount} shift{staff.shiftCount === 1 ? '' : 's'} / {staff.eventCount} event{staff.eventCount === 1 ? '' : 's'}{staff.conflictCount ? ` · ${staff.conflictCount} overlap${staff.conflictCount === 1 ? '' : 's'}` : ''}</span>)}</div></div> : null}{staffingCoverageQuery.data?.coverage.events.map((coverage) => <div key={coverage.eventId || 'unassigned'} className="flex items-center justify-between gap-2 rounded border border-border p-2 text-sm"><span><strong>{coverage.eventTitle}</strong><span className="ml-2 text-fg-muted">{coverage.shifts.length} shift{coverage.shifts.length === 1 ? '' : 's'}</span></span><span className="flex items-center gap-2 text-fg-muted">{coverage.staffCount} staff{coverage.eventId && <Button size="xs" variant="outline" onClick={() => { window.location.hash = `#/events/${coverage.eventId}?tab=staff`; }}>Manage</Button>}</span></div>) || <p className="text-sm text-fg-muted">No staffing shifts are scheduled yet.</p>}</div>}</CardContent></Card>
