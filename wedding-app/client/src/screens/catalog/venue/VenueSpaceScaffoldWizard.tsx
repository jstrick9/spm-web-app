...
export function VenueSpaceScaffoldWizard({ orgId, onSelectVenue }: { orgId: string; onSelectVenue: (venue: any) => void }) {
  ...
  async function uploadUnderlay(venueId: string, file: File) {
    if (!['image/png', 'image/jpeg'].includes(file.type)) { toast({ title: 'Unsupported underlay', description: 'Upload a PNG or JPEG reference plan in this phase.', variant: 'destructive' }); return; }
