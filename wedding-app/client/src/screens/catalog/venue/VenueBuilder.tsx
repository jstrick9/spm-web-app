...
export function VenueBuilder({ orgId }: Props) {
  ...
  const { data: templateUsageLayouts } = useQuery({
    queryKey: ['template-usage-layouts', orgId],
  ...
  const templateMutation = useMutation({
    mutationFn: () => {
      ...
      const payload = { name: templateName.trim() || `${selectedVenue.name} ${templateMoment} template`, visible: true,
      ...
      return editingTemplateId ? sdk.catalog.update(editingTemplateId, payload) : sdk.catalog.create(orgId, 'template', payload);
    },
    onSuccess: () => { setTemplateName(''); setTemplateInventoryIds([]); setEditingTemplateId(null); setTemplateEditorOpen(false); qc.invalidateQueries({ queryKey: ['catalog', orgId, 'template'] }); toast({ title: editingTemplateId ? 'Venue template updated' : 'Venue template published', description: 'Couples can now use this approved template as an editable proposal.', variant: 'success' }); },
    onError: (e: any) => toast({ title: 'Could not publish template', description: e.message, variant: 'destructive' })
  });

  const duplicateTemplateMutation = useMutation({
    mutationFn: (template: any) => sdk.catalog.create(orgId, 'template', { name: `${template.name} copy`, visible: false, spec: typeof template.spec === 'string' ? JSON.parse(template.spec || '{}') : (template.spec || {}) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['catalog', orgId, 'template'] }); toast({ title: 'Template copied as a draft', description: 'Edit and restore it when it is ready for couples.', variant: 'success' }); },
    onError: (e: any) => toast({ title: 'Could not copy template', description: e.message, variant: 'destructive' }),
  });

  const archiveTemplateMutation = useMutation({
    mutationFn: ({ template, visible }: { template: any; visible: boolean }) => sdk.catalog.update(template.id, { visible }),
    onSuccess: (_result, variables) => { qc.invalidateQueries({ queryKey: ['catalog', orgId, 'template'] }); toast({ title: variables.visible ? 'Template restored' : 'Template archived', description: variables.visible ? 'It is available to couples again when its venue space is approved.' : 'It is no longer available to couples.', variant: 'success' }); },
    onError: (e: any) => toast({ title: 'Could not update template', description: e.message, variant: 'destructive' }),
  });

  const editTemplate = (template: any) => {
    const spec = typeof template.spec === 'string' ? (() => { try { return JSON.parse(template.spec); } catch { return {}; } })() : (template.spec || {});
    if (!selectedVenue || selectedVenue.id !== spec.venueId) { toast({ title: "Select this template’s approved venue space first", description: 'Templates retain their approved structural space.', variant: 'destructive' }); return; }
    setEditingTemplateId(template.id); setTemplateName(template.name); setTemplateMoment(spec.weddingMoment || 'reception'); setTemplateService(spec.serviceStyle || 'plated'); setTemplateMinGuests(String(spec.minGuests || 1)); setTemplateMaxGuests(String(spec.maxGuests || selectedVenue.capacity || '')); setTemplateCategories(Array.isArray(spec.allowedObjectCategories) ? spec.allowedObjectCategories : ['tables', 'chairs', 'decor', 'service', 'ceremony']); setTemplateInventoryIds(Array.isArray(spec.allowedInventoryItemIds) ? spec.allowedInventoryItemIds : []); setTemplateEditorOpen(true);
  };
  ...
  const templateUsage = (templateId: string) => (templateUsageLayouts?.layouts || []).filter((layout: any) => {
    try { const payload = typeof layout.payload === 'string' ? JSON.parse(layout.payload || '{}') : (layout.payload || {}); return payload.templateId === templateId; } catch { return false; }
  });
  ...
  return (
    <div className="flex flex-col gap-4">
       ...
       {templateEditorOpen && <section aria-label="Venue template editor" className="rounded-lg border border-primary/30 bg-primary/5 p-4 shadow-sm">
         ...
         <div className="mt-4 flex justify-end gap-2"><Button variant="outline" onClick={() => setTemplateEditorOpen(false)}>Cancel</Button><Button isLoading={templateMutation.isPending} onClick={() => templateMutation.mutate()}>{editingTemplateId ? 'Save template' : 'Publish template'}</Button></div>
       </section>}

       <section aria-label="Venue template library" className="rounded-lg border border-border bg-surface p-4"><div className="flex items-baseline justify-between gap-3"><div><h3 className="font-semibold">Venue template library</h3><p className="text-sm text-fg-muted">Only active templates linked to approved spaces appear in the couple gallery.</p></div><span className="text-sm text-fg-muted">{venueTemplates?.items?.filter((template: any) => template.visible).length ?? 0} active</span></div><div className="mt-3 grid gap-2 md:grid-cols-2">{venueTemplates?.items?.length ? venueTemplates.items.map((template: any) => { const spec = typeof template.spec === 'string' ? (() => { try { return JSON.parse(template.spec); } catch { return {}; } })() : (template.spec || {}); return <div key={template.id} className="flex items-center justify-between gap-3 rounded border border-border p-3 text-sm"><div className="flex min-w-0 items-center gap-3"><svg aria-label={`${template.name} layout preview`} viewBox="0 0 800 600" className="h-16 w-24 shrink-0 rounded border border-border bg-surface-2">{(spec.masterLayout?.walls || spec.masterLayout?.lines || []).slice(0, 20).map((wall: any, index: number) => <polyline key={wall.id || index} points={(wall.points || []).reduce((pairs: string[], point: number, pointIndex: number, values: number[]) => pointIndex % 2 === 0 ? [...pairs, `${point},${values[pointIndex + 1] ?? point}`] : pairs, []).join(' ')} fill="none" stroke="currentColor" strokeWidth="10" />)}{(spec.masterLayout?.zones || []).slice(0, 8).map((zone: any) => <rect key={zone.id} x={zone.x || 0} y={zone.y || 0} width={zone.width || 30} height={zone.height || 30} fill="currentColor" opacity="0.25" />)}</svg><div className="min-w-0"><strong>{template.name}</strong><p className="text-fg-muted">{spec.weddingMoment || 'Wedding layout'} · {spec.serviceStyle || 'Flexible service'} · {spec.minGuests ?? '—'}–{spec.maxGuests ?? '—'} guests</p><p className="text-xs text-fg-muted">{template.visible ? 'Active in couple gallery' : 'Archived'} · {(spec.allowedObjectCategories || []).join(', ') || 'No object permissions set'}</p><p className="text-xs text-fg-muted">Used by {templateUsage(template.id).length} event proposal{templateUsage(template.id).length === 1 ? '' : 's'}{templateUsage(template.id).some((layout: any) => layout.approval_status === 'pending') ? ' · venue review pending' : ''}</p></div></div><div className="flex gap-1"><Button size="xs" variant="ghost" disabled={!template.visible} onClick={() => editTemplate(template)}>Edit</Button><Button size="xs" variant="ghost" isLoading={duplicateTemplateMutation.isPending} onClick={() => duplicateTemplateMutation.mutate(template)}>Duplicate</Button><Button size="xs" variant="outline" isLoading={archiveTemplateMutation.isPending} onClick={() => archiveTemplateMutation.mutate({ template, visible: !template.visible })}>{template.visible ? 'Archive' : 'Restore'}</Button></div></div>; }) : <p className="text-sm text-fg-muted">Save an approved layout as a template to start your gallery.</p>}</div></section>
