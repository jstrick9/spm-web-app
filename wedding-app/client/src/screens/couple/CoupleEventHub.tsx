        ...
        <div className={`space-y-5 pb-24 md:pb-0 ${largeTextMode ? 'text-lg' : ''} ${accessibilityMode ? 'contrast-more' : ''}`} onInputCapture={() => setHasUnsavedDraft(true)}>
          ...
          <Card id="couple-documents" className="border-brand/20">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-2 lg:grid-cols-2">{(documentsQuery.data?.documents || []).slice(0, 8).map((doc) => <div key={doc.id} className="rounded-lg border border-border bg-surface-2 p-3 text-sm"><div className="flex items-start justify-between gap-2"><div><strong>{doc.filename}</strong><p className="text-xs text-fg-muted">{doc.category} · {doc.visibility} · v{doc.version}</p>{doc.extractedSummary && <p className="mt-1 whitespace-pre-wrap text-xs text-fg-muted">{doc.extractedSummary}</p>}</div><Badge variant={doc.approvalStatus === 'approved' ? 'success' : doc.approvalStatus === 'changes_requested' ? 'warning' : 'outline'}>{doc.approvalStatus}</Badge><div className="flex shrink-0 gap-1"><Button size="xs" variant="outline" onClick={() => downloadAuth(doc.url, doc.filename)}>Open</Button><Button size="xs" variant="ghost" onClick={() => documentDeleteMutation.mutate(doc.id)} aria-label={`Delete document ${doc.filename}`}><Trash2 className="h-3.5 w-3.5" /></Button></div></div></div>)}</div>
