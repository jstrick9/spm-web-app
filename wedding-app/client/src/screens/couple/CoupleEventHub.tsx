...
export function CoupleEventHub({ eventId }: { eventId: string }) {
  ...
  const attentionCards = [
    ...
    ...(rsvpDeadline && daysUntil(rsvpDeadline) !== null && daysUntil(rsvpDeadline)! <= 14 && rsvpPct < 85 ? [{ title: 'RSVP deadline approaching', detail: `${rsvpPct}% responses received; deadline ${rsvpDeadline}.`, severity: 'warning' as const }] : []),
      ...
      <PageBody>
        <div className={`space-y-5 pb-24 md:pb-0 ${largeTextMode ? 'text-lg' : ''} ${accessibilityMode ? 'contrast-more' : ''}`} onInputCapture={() => setHasUnsavedDraft(true)}>
            ...
            <CardContent className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                ...
                <div className="rounded-2xl border border-border bg-surface/90 p-5 shadow-sm">
                  ...
                  <div className="mt-2 text-3xl font-bold text-brand">{readiness}%</div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Users className="h-4 w-4 text-brand" /> RSVP progress</CardTitle><CardDescription>Client-safe guest response summary.</CardDescription></CardHeader><CardContent><div className="text-3xl font-bold text-brand">{rsvpPct}%</div><p className="mt-1 text-xs text-fg-muted">{rsvpResolved} of {totalGuests || '—'} guests responded. RSVP deadline: {rsvpDeadline || 'TBD'}.</p></CardContent></Card>
          ...
          <Card id="couple-documents" className="border-brand/20">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-2 lg:grid-cols-2">{(documentsQuery.data?.documents || []).slice(0, 8).map((doc) => <div key={doc.id} className="rounded-lg border border-border bg-surface-2 p-3 text-sm"><div className="flex items-start justify-between gap-2"><div><strong>{doc.filename}</strong><p className="text-xs text-fg-muted">{doc.category} · {doc.visibility} · v{doc.version}</p>{doc.extractedSummary && <p className="mt-1 whitespace-pre-wrap text-xs text-fg-muted">{doc.extractedSummary}</p>}</div><Badge variant={doc.approvalStatus === 'approved' ? 'success' : doc.approvalStatus === 'changes_requested' ? 'warning' : 'outline'}>{doc.approvalStatus}</Badge></div></div>)}</div>
          ...
          <Card className="border-brand/20">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-3 lg:grid-cols-3"><div className="rounded-lg border border-border bg-surface-2 p-3 text-xs"><strong>RSVP rules</strong><p>Deadline: {guestPortalQuery.data?.portal.rsvpDeadline || rsvpDeadline || 'TBD'}</p><p>Edit window: {guestPortalQuery.data?.portal.editWindowDays ?? 'venue default'} day(s)</p><p>Confirmation email preview: Thanks for your RSVP. Meal, dietary, accessibility, travel, and lodging details are saved.</p></div><div className="rounded-lg border border-border bg-surface-2 p-3 text-xs"><strong>Mobile QA checklist</strong><ul className="mt-1 list-disc pl-4">{(guestPortalQuery.data?.mobileQa || []).slice(0, 4).map((item) => <li key={item}>{item}</li>)}</ul></div><div className="rounded-lg border border-border bg-surface-2 p-3 text-xs"><strong>Secure links / QR</strong><p>QR payload: {guestPortalQuery.data?.qrPayload || `WVI-RSVP:${eventId}`}</p><Button size="xs" variant="outline" onClick={generateFirstGuestLink}>Copy first guest tokenized link</Button></div></div>
