...
export function CoupleEventHub({ eventId }: { eventId: string }) {
  ...
  // Docs and timeline cards were hard-capped (8/10) with no way to see the
  // rest — couples with more items silently lost access to older ones.
        ...
        <div className={`space-y-5 pb-24 md:pb-0 ${largeTextMode ? 'text-lg' : ''} ${accessibilityMode ? 'contrast-more' : ''}`} onInputCapture={() => setHasUnsavedDraft(true)}>
          ...
          <Card id="couple-documents" className="border-brand/20">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-2 lg:grid-cols-2">{documentsQuery.isError ? <SectionLoadError label="Documents" onRetry={() => void documentsQuery.refetch()} /> : (<>{(documentsQuery.data?.documents || []).slice(0, showAllDocs ? undefined : 8).map((doc) => <div key={doc.id} className="rounded-lg border border-border bg-surface-2 p-3 text-sm"><div className="flex items-start justify-between gap-2"><div><strong>{doc.filename}</strong><p className="text-xs text-fg-muted">{doc.category} · {doc.visibility} · v{doc.version}</p>{doc.extractedSummary && <p className="mt-1 whitespace-pre-wrap text-xs text-fg-muted">{doc.extractedSummary}</p>}</div><Badge variant={doc.approvalStatus === 'approved' ? 'success' : doc.approvalStatus === 'changes_requested' ? 'warning' : 'outline'}>{doc.approvalStatus}</Badge><div className="flex shrink-0 gap-1"><Button size="xs" variant="outline" onClick={() => downloadAuth(doc.url, doc.filename)}>Open</Button><Button size="xs" variant="outline" onClick={() => startEditDocument(doc)} aria-label={`Edit details of ${doc.filename}`}>Edit details</Button><Button size="xs" variant="outline" onClick={() => documentVersionFileInputs.current?.[doc.id]?.click()} disabled={documentVersionUploadMutation.isPending} aria-label={`Upload new version of ${doc.filename}`}><Upload className="h-3.5 w-3.5" /> New version</Button><input ref={(el) => { if (el) documentVersionFileInputs.current[doc.id] = el; }} type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" className="hidden" aria-label={`Choose a new version of ${doc.filename}`} onChange={handleDocumentVersionChosen(doc.id)} /><Button size="xs" variant="ghost" onClick={() => documentDeleteMutation.mutate(doc.id)} aria-label={`Delete document ${doc.filename}`}><Trash2 className="h-3.5 w-3.5" /></Button></div></div></div>)}{((documentsQuery.data?.documents || []).length) > 8 && <div className="text-center"><Button size="xs" variant="ghost" onClick={() => setShowAllDocs((v) => !v)}>{showAllDocs ? "Show fewer" : `Show all ${(documentsQuery.data?.documents || []).length} documents`}</Button></div>}</>)}</div>
          ...
          <Card id="couple-timeline-review" className="border-brand/20">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-2 lg:grid-cols-2">{(coupleTimelineQuery.data?.items || []).slice(0, showAllTimeline ? undefined : 10).map((item) => <div key={item.id} className="rounded-lg border border-border bg-surface-2 p-3 text-sm"><div className="flex items-start justify-between gap-2"><div><strong>{item.title}</strong><p className="text-xs text-fg-muted">{item.startsAt} · {item.location || 'Location TBD'} · {item.category}</p>{item.notes && <p className="mt-1 text-xs text-fg-muted">{item.notes}</p>}</div><Button size="xs" variant="ghost" onClick={() => requestTimelineChange(item.id)}>Request change</Button></div></div>)}{((coupleTimelineQuery.data?.items || []).length) > 10 && <div className="text-center"><Button size="xs" variant="ghost" onClick={() => setShowAllTimeline((v) => !v)}>{showAllTimeline ? "Show fewer" : `Show all ${(coupleTimelineQuery.data?.items || []).length} timeline items`}</Button></div>}</div>
