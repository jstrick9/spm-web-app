...
export function CouplePostEventCloseout({ eventId, venueContactEmail, coupleNames }: { eventId: string; venueContactEmail?: string; coupleNames?: string }) {
  ...
  const downloadAuth = async (url: string, filename?: string) => {
    try {
      ...
      toast({ title: 'Could not download packet', description: err?.message || 'Please try again.', variant: 'destructive' });
    }
  };
