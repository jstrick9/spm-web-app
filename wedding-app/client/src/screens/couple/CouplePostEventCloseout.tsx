...
export function CouplePostEventCloseout({ eventId, venueContactEmail, coupleNames }: { eventId: string; venueContactEmail?: string; coupleNames?: string }) {
  ...
  const lostItemMutation = useMutation({
    mutationFn: () => sdk.couple.reportLostItem(eventId, { itemDescription: window.prompt('Describe the lost item') || '', lastSeenLocation: window.prompt('Where was it last seen?', '') || undefined, contactPreference: 'email', contactValue: venueContactEmail }),
  ...
  const reviewWorkflowMutation = useMutation({
    mutationFn: () => sdk.couple.submitReviewWorkflow(eventId, { platform: 'google', rating: Number(postEventSurveyDraft.overallRating || 5), testimonial: postEventSurveyDraft.publicTestimonial || window.prompt('Optional testimonial/review text') || undefined, permissionToPublish: true, reviewerName: coupleNames }),
