...
export function EventFeedbackTab({ eventId }: Props) {
  ...
  const followUpMutation = useMutation({
    mutationFn: (message: string) => sdk.couple.queuePostEventFollowUp(eventId, { requestIds: selectedCloseoutIds, channel: 'email', message }),
    ...
    <div className="space-y-6">
      ...
      <Card className="border-brand/20 bg-brand-soft/10">
        ...
        <CardContent className="pt-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface p-3 text-xs"><strong>{selectedCloseoutIds.length}</strong><span>selected</span><Button size="xs" variant="outline" disabled={selectedCloseoutIds.length === 0 || bulkCloseoutMutation.isPending} onClick={async () => { const assignedTo = await ask({ title: 'Assign selected requests', label: 'Coordinator email', required: true }); if (assignedTo) bulkCloseoutMutation.mutate({ assignedTo, slaDays: 3, note: 'Assigned with 3-day SLA from venue queue.' }); }}>Assign + 3-day SLA</Button><Button size="xs" variant="outline" disabled={selectedCloseoutIds.length === 0 || followUpMutation.isPending} onClick={async () => { const message = await ask({ title: 'Queue email follow-up', label: 'Follow-up message', multiline: true, required: true, defaultValue: 'Hi! We are following up on your post-event closeout request and will update you shortly.' }); if (message) followUpMutation.mutate(message); }}>Queue email follow-up</Button><Button size="xs" disabled={selectedCloseoutIds.length === 0 || bulkCloseoutMutation.isPending} onClick={() => bulkCloseoutMutation.mutate({ status: 'completed', note: 'Completed in bulk from venue queue.' })}>Bulk complete</Button></div>
