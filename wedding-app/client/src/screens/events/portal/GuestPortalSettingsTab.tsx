...
export function GuestPortalSettingsTab({ eventId }: Props) {
  ...
  const [guestDirectoryEnabled, setGuestDirectoryEnabled] = useState(false);
  ...
  // Creating sub-events (rehearsal dinner, welcome party, brunch…) was
  // previously impossible from the UI — the server + SDK supported it but
  // no screen called createSubEvent. Guests only ever saw sub-events in
  // their itinerary if the venue created them by API.
  const createSubEventMutation = useMutation({
    mutationFn: (input: { title: string; startsAt: string; inviteOnly?: boolean }) =>
      sdk.events.createSubEvent(eventId, {
        title: input.title,
        startsAt: input.startsAt,
        inviteOnly: input.inviteOnly,
        metadata: {},
      }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["sub-events", eventId, "guest-metadata"] }); toast({ title: "Sub-event created", description: "Fill in guest-facing details below.", variant: "success" }); },
    onError: (e: any) => toast({ title: "Could not create sub-event", description: e.message, variant: "destructive" }),
  });
  ...
  useEffect(() => {
    if (data && data.config) {
      ...
      setGuestDirectoryEnabled(parsedConfig.allowGenericGuestDirectory === true);
    ...
    mutationFn: () => {
      const payload: any = {
        ...
        config: {
          ...
          allowGenericGuestDirectory: guestDirectoryEnabled,
