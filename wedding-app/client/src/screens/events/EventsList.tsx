...
function StatusDefinitionStrip() {
  return (
...
function parseEventMetadata(event: SdkEvent): Record<string, any> {
  try {
...
function operationalStatusFor(event: SdkEvent): {
  label: string;
...
function eventComplexityScore(event: SdkEvent): {
  score: number;
  reasons: string[];
} {
  const metadata = parseEventMetadata(event);
...
function eventDaysUntil(event: SdkEvent): number | null {
...
function applyManagerPipelineFilter(
  events: SdkEvent[],
...
function ManagerPipelineControls({
  active,
  onChange,
  events,
}: {
...
function SalesToOperationsHandoff({ events }: { events: SdkEvent[] }) {
...
function ManagerAssignmentBoard({ events }: { events: SdkEvent[] }) {
...
function safeMetadata(raw: string): Record<string, any> {
  try {
...
function eventSetupCompletion(event: SdkEvent): number {
  const metadata = safeMetadata(event.metadata);
