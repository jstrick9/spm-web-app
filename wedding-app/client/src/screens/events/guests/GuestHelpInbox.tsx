...
export function GuestHelpInbox({ eventId }: { eventId: string }) {
  ...
  const reply = useMutation({
    mutationFn: ({ id, channel, closeRequest, message }: { id: string; channel: 'email' | 'sms' | 'in_app'; closeRequest?: boolean; message: string }) => sdk.guests.replyGuestHelpRequest(eventId, id, { channel, closeRequest, message }),
  ...
  const replyWith = async (id: string, channel: 'email' | 'sms', closeRequest: boolean) => {
    const message = await ask({ title: closeRequest ? 'Reply and resolve' : `Reply via ${channel === 'sms' ? 'SMS' : 'email'}`, label: 'Reply message to guest', multiline: true, required: true, defaultValue: 'Thanks for reaching out. We are checking your invitation details and will follow up shortly.' });
    if (!message) return;
    reply.mutate({ id, channel, closeRequest, message });
  };

  const assign = async (id: string) => {
    const assignedTo = await ask({ title: 'Assign this request', label: 'Coordinator email', required: true });
