...
function centerDistance(a: any, b: any): number {
  return Math.hypot(Number(a?.x ?? 0) - Number(b?.x ?? 0), Number(a?.y ?? 0) - Number(b?.y ?? 0));
}

type FloorWalkCheckId = 'exits' | 'ada' | 'power' | 'tables' | 'vendor_zones' | 'rain_plan' | 'fire_marshal' | 'accessibility';

interface ManagerLayoutOpsState {
...
const FLOOR_WALK_CHECKS: Array<{ id: FloorWalkCheckId; label: string; detail: string }> = [
...
const DEFAULT_MANAGER_LAYOUT_OPS: ManagerLayoutOpsState = {
  floorWalkChecks: {},
  varianceEvidence: [],
  rainPlanActive: false,
};

function managerLayoutOpsFromBackend(ops: any): ManagerLayoutOpsState {
...
function MobileReportSection({ title, rows }: { title: string; rows: Array<[string, string]> }) {
  return (
...
function MiniLayoutMetric({ label, value, detail }: { label: string; value: string | number; detail: string }) {
