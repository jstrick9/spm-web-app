...
export function generateWeddingPackage(kind: WeddingLayoutPackage, guestCount: number, startIndex = 0): any[] {
  ...
  if (kind === 'ceremony') {
    objects.push({ id: id('ceremony-aisle', 0), type: 'decor', x: 400, y: 320, label: 'Ceremony aisle', width: 44, height: 300, shape: 'rect', color: '#f3f4f6' }, { id: id('ceremony-arch', 0), type: 'decor', x: 400, y: 90, label: 'Ceremony focal point', width: 130, height: 34, shape: 'rect', color: '#dcfce7' });
    for (let index = 0; index < guests; index++) objects.push({ id: id('ceremony-chair', index), type: 'chair', x: 270 + (index % 2) * 260 + (Math.floor(index / 2) % 7) * 27, y: 170 + Math.floor(index / 14) * 38, radius: 10, label: 'Ceremony chair' });
