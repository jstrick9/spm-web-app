...
export function GuestPortalHome({
  ...
  return (
    <>
      ...
      <GuestReminderPreferences eventId={eventId} activeGuest={activeGuest} guestToken={guestToken} palette={palette} guestReminders={guestReminders} />
      ...
      <GuestAccessibilityCareCenter eventId={eventId} activeGuest={activeGuest} guestToken={guestToken} palette={palette} guestCare={guestCare} guestTravel={guestTravel} />
      <GuestFaqEtiquette eventId={eventId} activeGuest={activeGuest} guestToken={guestToken} palette={palette} branding={branding} guestFaq={guestFaq} config={config} />
...
function GuestEventDashboard({ info, guestHome, activeGuest, palette, setActiveTab, saveGuestEventDetails }: {
  ...
  return (
    <Card style={{ background: palette.surface, borderColor: palette.border }}>
      <CardHeader>
        <CardTitle className="font-display text-2xl">{t('home.title')}</CardTitle>
        <CardDescription style={{ color: palette.fgMuted }}>{t('home.subtitle')}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 text-sm">
        <div className="grid gap-3 sm:grid-cols-4">
          <SummaryTile label={t('home.eventType')} value={String(guestHome?.eventType || info.eventType || t('shell.unknownEventType')).replace(/_/g, ' ')} palette={palette} />
          <SummaryTile label={t('home.date')} value={info.startDate ? formatDateOnly(info.startDate) : t('common.tbd')} palette={palette} />
          <SummaryTile label={t('home.location')} value={guestHome?.locationSummary || info.locationSummary || t('home.venuePending')} palette={palette} />
          <SummaryTile label={t('home.lastUpdated')} value={guestHome?.lastUpdatedAt || info.lastUpdatedAt ? new Date((guestHome?.lastUpdatedAt || info.lastUpdatedAt)!).toLocaleDateString() : t('common.recently')} palette={palette} />
        </div>
        {(guestHome?.rsvpDeadline || info.rsvpDeadline || guestHome?.editWindowDays !== null) && <div className="rounded-xl border border-warning/30 bg-warning-soft/20 p-3 text-warning"><strong>{t('home.rsvpDeadline')}</strong> {formatDateOnly(guestHome?.rsvpDeadline || info.rsvpDeadline) || t('home.checkInvitation')}{guestHome?.editWindowDays != null ? t('home.editsClose', { days: guestHome.editWindowDays }) : ''}</div>}
        <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-6">
          <Button type="button" aria-label={t('home.openRsvp')} onClick={() => setActiveTab('rsvp')}>{t('shell.rsvp')}</Button>
          <Button type="button" variant="outline" onClick={() => document.getElementById('guest-schedule-info')?.scrollIntoView({ behavior: 'smooth' })}>{t('home.schedule')}</Button>
          <Button type="button" variant="outline" onClick={() => document.getElementById('guest-travel-info')?.scrollIntoView({ behavior: 'smooth' })}>{t('home.directions')}</Button>
          <Button type="button" variant="outline" onClick={() => document.getElementById('guest-lodging-info')?.scrollIntoView({ behavior: 'smooth' })}>{t('home.lodging')}</Button>
          <Button type="button" variant="outline" onClick={() => document.getElementById('guest-faq-info')?.scrollIntoView({ behavior: 'smooth' })}>{t('home.faq')}</Button>
          <Button type="button" variant="outline" onClick={saveGuestEventDetails}>{t('home.saveDetails')}</Button>
        </div>
        {activeGuest && <div className="rounded-xl border p-3" style={{ borderColor: palette.border, background: palette.accentSoft }}><strong>{t('home.personalizedItinerary')}</strong><p style={{ color: palette.fgMuted }}>{t('home.rsvpAs', { name: activeGuest.fullName })}{activeGuest.tableAssignment ? t('home.table', { table: activeGuest.tableAssignment }) : ''}{activeGuest.seatAssignment ? t('home.seat', { seat: activeGuest.seatAssignment }) : ''}{activeGuest.roomAssignment ? t('home.room', { room: activeGuest.roomAssignment }) : ''}.</p></div>}
        {(guestHome?.changeNotices || []).length > 0 && <div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong>{t('shell.whatChanged')}</strong><div className="mt-2 space-y-2">{(guestHome?.changeNotices || []).slice(0, 4).map((notice) => <div key={notice.id} className="rounded-lg border p-2 text-xs" style={{ borderColor: palette.border }}><div className="font-bold">{notice.title}</div><p style={{ color: palette.fgMuted }}>{notice.body}</p><span style={{ color: palette.fgMuted }}>{new Date(notice.updatedAt).toLocaleString()}</span></div>)}</div></div>}
      </CardContent>
    </Card>
  );
}
...
function GuestStartHere({ palette, guestToken, portalAccess, lookupQuery, setLookupQuery, lookupEmail, setLookupEmail, lookupGuest, lookupMessage, lookupResults, onSelectLookupGuest, resendSecureLink, requestGuestHelp }: {
  palette: Palette;
  guestToken: string;
  portalAccess: PortalInfoResponse['access'] | null;
  lookupQuery: string;
  ...
  return (
    <Card style={{ background: palette.surface, borderColor: palette.border }}>
      <CardHeader>
        <CardTitle className="font-display text-2xl">{t('home.startHereTitle')}</CardTitle>
        <CardDescription style={{ color: palette.fgMuted }}>{t('home.startHereSubtitle')}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 text-sm">
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong>{t('home.step1Title')}</strong><p style={{ color: palette.fgMuted }}>{t('home.step1')}</p></div>
          <div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong>{t('home.step2Title')}</strong><p style={{ color: palette.fgMuted }}>{t('home.step2')}</p></div>
          <div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong>{t('home.step3Title')}</strong><p style={{ color: palette.fgMuted }}>{t('home.step3')}</p></div>
        </div>
        <div className="flex flex-wrap items-center gap-2"><Label className="text-xs uppercase tracking-wider">{t('home.language')}</Label><select value={lang} onChange={(e) => setLang(e.target.value as PortalLanguage)} className="h-9 rounded-md border px-2 text-sm" style={{ borderColor: palette.border, background: palette.surface }} aria-label={t('home.language')}><option value="en">English</option><option value="es">Español</option><option value="fr">Français</option><option value="zh">中文</option></select>{guestToken ? <Badge variant="success">{t('home.secureLink')}</Badge> : <Badge variant="outline">{t('home.genericLookup')}</Badge>}</div>
        ...
        {!guestToken && !portalAccess?.endsAt && <p className="rounded-lg border border-warning/30 bg-warning-soft/20 p-3 text-xs text-warning">{t('home.privacyWarning')}</p>}
        {!guestToken && <div className="rounded-xl border p-3 space-y-3" style={{ borderColor: palette.border, background: palette.accentSoft }}><strong>{t('home.findInvitation')}</strong><div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><Input value={lookupQuery} onChange={(e) => setLookupQuery(e.target.value)} placeholder={t('home.lookupName')} aria-label={t('home.lookupName')} /><Input value={lookupEmail} onChange={(e) => setLookupEmail(e.target.value)} placeholder={t('home.lookupEmail')} aria-label={t('home.lookupEmail')} /><Button type="button" onClick={() => lookupGuest()} disabled={lookupQuery.trim().length < 2}>{t('home.lookUp')}</Button></div>{lookupMessage && <p className="text-xs" style={{ color: palette.fgMuted }}>{lookupMessage}</p>}{lookupResults.length > 0 && <div className="space-y-1">{lookupResults.map((match) => <button key={match.id} type="button" className="block w-full rounded-lg border p-2 text-left text-xs" style={{ borderColor: palette.border, background: palette.surface }} onClick={() => onSelectLookupGuest(match)}>{t('home.possibleMatch')} <strong>{match.label}</strong>{match.partyName ? ` · ${match.partyName}` : ''}{t('home.secureLinkRecommended')}</button>)}</div>}<div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => resendSecureLink()}>{t('home.requestLink')}</Button><Button type="button" size="sm" variant="ghost" onClick={() => requestGuestHelp('cannot_find_name')}>{t('home.cannotFindName')}</Button><Button type="button" size="sm" variant="ghost" onClick={() => requestGuestHelp('wrong_guest')}>{t('home.linkNotMe')}</Button></div></div>}
        {guestToken && <div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => requestGuestHelp('wrong_guest')}>{t('home.linkNotMe')}</Button><Button type="button" size="sm" variant="ghost" onClick={() => requestGuestHelp('expired_or_revoked')}>{t('home.linkProblem')}</Button></div>}
      </CardContent>
    </Card>
  );
}
...
function GuestVenueMessages({ palette, replies, emptyState }: { palette: Palette; replies: Array<{ id: string; body: string; channel: string; sentByLabel: string; createdAt: string }>; emptyState: string }) {
  const { t } = useI18n();
  return <Card style={{ background: palette.surface, borderColor: palette.border }}><CardHeader><CardTitle className="font-display text-xl">{t('home.messagesTitle')}</CardTitle><CardDescription style={{ color: palette.fgMuted }}>{t('home.messagesSubtitle')}</CardDescription></CardHeader><CardContent className="space-y-2 text-sm">{replies.length ? replies.slice(0, 5).map((reply) => <div key={reply.id} className="rounded-lg border p-3" style={{ borderColor: palette.border, background: palette.accentSoft }}><div className="flex flex-wrap items-center justify-between gap-2"><strong>{reply.sentByLabel || 'Venue team'}</strong><Badge variant="outline">{reply.channel}</Badge></div><p className="mt-1 whitespace-pre-wrap">{reply.body}</p><p className="mt-1 text-xs" style={{ color: palette.fgMuted }}>{new Date(reply.createdAt).toLocaleString()}</p></div>) : <p className="rounded-lg border border-dashed p-3 text-sm" style={{ borderColor: palette.border, color: palette.fgMuted }}>{emptyState}</p>}</CardContent></Card>;
}
...
export function GuestMemoryPhotoSharing({ eventId, activeGuest, guestToken, palette, guestPostEvent }: { eventId: string; activeGuest?: PortalGuestEntry; guestToken: string; palette: Palette; guestPostEvent: PortalInfoResponse['guestPostEvent'] | null }) {
  ...
  const submitPhoto = async () => {
    if (!photoConsent) return setPhotoStatus(t('home.memoriesConsent'));
    if (!photoUrl.trim() && !caption.trim()) return setPhotoStatus(t('home.memoriesNeedContent'));
    ...
    catch { setPhotoStatus(t('home.memoriesFailed')); }
  };
  const submitFeedback = async () => {
    ...
    catch { setFeedbackStatus(t('home.memoriesFeedbackFailed')); }
  };
  if (!post.enabled) return null;
  return <Card id="guest-memory-photo-sharing" style={{ background: palette.surface, borderColor: palette.border }} aria-label={t('home.memoriesTitle')}><CardHeader><CardTitle className="font-display text-2xl flex items-center gap-2"><Images className="h-5 w-5" /> {t('home.memoriesTitle')}</CardTitle><CardDescription>{post.afterEvent ? post.thankYouMessage : t('home.memoriesSubtitle')}</CardDescription></CardHeader><CardContent className="space-y-4 text-sm"><div className="rounded-xl border p-3" style={{ borderColor: palette.border, background: palette.accentSoft }}><strong>{post.thankYouTitle}</strong><p className="mt-1" style={{ color: palette.fgMuted }}>{post.thankYouMessage}</p></div>{post.galleryDocuments && post.galleryDocuments.length > 0 && <div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong className="flex items-center gap-2"><Images className="h-4 w-4" /> {t('home.memoriesPhotos')}</strong><p className="mt-1 text-xs" style={{ color: palette.fgMuted }}>{t('home.memoriesPhotosCopy')}</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{post.galleryDocuments.map((doc) => <a key={doc.id} href={doc.url} target="_blank" rel="noreferrer noopener" className="rounded-xl border p-3 font-bold underline" style={{ borderColor: palette.border }}><ExternalLink className="inline h-4 w-4 mr-1" />{doc.filename}{doc.notes ? <span className="block mt-1 text-xs font-normal no-underline" style={{ color: palette.fgMuted }}>{doc.notes}</span> : null}</a>)}</div></div>}
{post.links.length > 0 && <div className="grid gap-2 sm:grid-cols-2">{post.links.map((link) => <a key={link.id} href={link.url} target="_blank" rel="noreferrer noopener" className="rounded-xl border p-3 font-bold underline" style={{ borderColor: palette.border }}><ExternalLink className="inline h-4 w-4 mr-1" />{link.label}{link.description && <p className="mt-1 text-xs font-normal no-underline" style={{ color: palette.fgMuted }}>{link.description}</p>}</a>)}</div>}{post.uploadEnabled && <div className="rounded-xl border p-3 space-y-2" style={{ borderColor: palette.border }}><strong className="flex items-center gap-2"><Camera className="h-4 w-4" /> {t('home.memoriesShare')}</strong><p className="text-xs" style={{ color: palette.fgMuted }}>{post.moderationCopy}</p><Input value={photoUrl} onChange={(e) => setPhotoUrl(e.target.value)} placeholder={t('home.memoriesPhotoPlaceholder')} aria-label={t('home.memoriesPhotoPlaceholder')} /><textarea value={caption} onChange={(e) => setCaption(e.target.value)} className="min-h-20 w-full rounded-md border p-2" style={{ borderColor: palette.border, background: palette.surface }} placeholder={t('home.memoriesCaptionPlaceholder')} aria-label={t('home.memoriesCaptionPlaceholder')} /><label className="flex items-start gap-2 text-sm"><input type="checkbox" checked={photoConsent} onChange={(e) => setPhotoConsent(e.target.checked)} /> {post.consentCopy}</label><div className="flex gap-2 items-center"><Button type="button" onClick={submitPhoto}>{t('home.memoriesSubmit')}</Button>{photoStatus && <span className="text-xs" style={{ color: palette.fgMuted }}>{photoStatus}</span>}</div></div>}{post.feedbackEnabled && <div className="rounded-xl border p-3 space-y-2" style={{ borderColor: palette.border }}><strong className="flex items-center gap-2"><Star className="h-4 w-4" /> {t('home.memoriesFeedback')}</strong><label className="text-sm">{post.npsQuestion}<Input type="number" min="0" max="10" value={score} onChange={(e) => setScore(Math.max(0, Math.min(10, Number(e.target.value) || 0)))} aria-label="Guest NPS score" /></label><textarea value={comment} onChange={(e) => setComment(e.target.value)} className="min-h-20 w-full rounded-md border p-2" style={{ borderColor: palette.border, background: palette.surface }} placeholder="Optional: what worked well or what would improve the guest experience?" aria-label="Guest post-event feedback comment" /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={contactConsent} onChange={(e) => setContactConsent(e.target.checked)} /> Venue/couple may contact me about this feedback.</label><div className="flex gap-2 items-center"><Button type="button" onClick={submitFeedback}>{t('home.memoriesSubmitFeedback')}</Button>{feedbackStatus && <span className="text-xs" style={{ color: palette.fgMuted }}>{feedbackStatus}</span>}</div></div>}</CardContent></Card>;
}
...
function GuestEventDayMobileMode({ eventId, info, activeGuest, guestToken, palette, guestTravel, guestDayOf, branding, setActiveTab }: { eventId: string; info: PortalInfoResponse['event']; activeGuest?: PortalGuestEntry; guestToken: string; palette: Palette; guestTravel: PortalInfoResponse['guestTravel'] | null; guestDayOf: PortalInfoResponse['guestDayOf'] | null; branding: PortalInfoResponse['branding'] | null; setActiveTab: (tab: ActiveTab) => void }) {
  ...
  const sendHelp = async (kind: 'running_late' | 'need_help') => {
    ...
    } catch { setStatus(t('home.dayOfHelpFailed')); }
  };
  const enablePush = async () => {
    if (typeof Notification === 'undefined') return setPushStatus(t('home.dayOfPushUnavailable'));
    const permission = await Notification.requestPermission();
    setPushStatus(permission === 'granted' ? t('home.dayOfPushEnabled') : t('home.dayOfPushDenied'));
  };
  return (
    <Card id="guest-event-day-mobile-mode" style={{ background: palette.surface, borderColor: palette.border }} aria-label={t('home.dayOfTitle')}>
      <CardHeader><CardTitle className="font-display text-2xl flex items-center gap-2"><Smartphone className="h-5 w-5" /> {t('home.dayOfTitle')}</CardTitle><CardDescription>{t('home.dayOfSubtitle', { title: dayOf.title })}</CardDescription></CardHeader>
      <CardContent className="space-y-4 text-sm">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          <DayOfTile label={t('home.dayOfAddress')} value={guestTravel?.venueAddress || info.locationSummary || t('home.dayOfAddressPending')} palette={palette} />
          <DayOfTile label={t('home.dayOfSchedule')} value={info.startDate ? formatDateOnly(info.startDate) : t('home.dayOfSchedulePending')} palette={palette} />
          <DayOfTile label={t('home.dayOfTableSeat')} value={activeGuest?.tableAssignment ? `${activeGuest.tableAssignment}${activeGuest.seatAssignment ? t('home.seat', { seat: activeGuest.seatAssignment }) : ''}` : t('home.dayOfSeatPending')} palette={palette} />
          <DayOfTile label={t('home.dayOfShuttle')} value={guestTravel?.shuttleSchedule || t('home.dayOfShuttlePending')} palette={palette} />
          <DayOfTile label={t('home.dayOfContact')} value={[dayOf.contactLabel, dayOf.contactPhone || dayOf.contactEmail].filter(Boolean).join(' · ') || t('home.dayOfContactPending')} palette={palette} />
          <button type="button" className="rounded-xl border p-3 text-left" style={{ borderColor: palette.border }} onClick={() => document.getElementById('guest-faq-info')?.scrollIntoView({ behavior: 'smooth' })}><div className="text-[10px] uppercase font-black tracking-widest" style={{ color: palette.fgMuted }}>{t('home.faq')}</div><p className="mt-1 font-semibold">{t('home.dayOfOpenFaq')}</p></button>
        </div>
        <div className="flex flex-wrap gap-2"><Button type="button" onClick={() => sendHelp('running_late')}>{t('home.dayOfRunningLate')}</Button><Button type="button" variant="outline" onClick={() => sendHelp('need_help')}>{t('home.dayOfNeedHelp')}</Button><Button type="button" variant="outline" onClick={() => setActiveTab('map')}>{t('home.dayOfFindSeat')}</Button><Button asChild type="button" variant="outline"><a href={dayOf.offlinePassUrl} download><Download className="h-4 w-4 mr-1" /> {t('home.dayOfOfflinePass')}</a></Button></div>
        <div className="rounded-xl border p-3" style={{ borderColor: palette.border, background: palette.accentSoft }}><strong className="flex items-center gap-2"><Bell className="h-4 w-4" /> {t('home.dayOfPushTitle')}</strong><p className="mt-1 text-xs" style={{ color: palette.fgMuted }}>{dayOf.pushCopy}</p><div className="mt-2 flex flex-wrap gap-2 items-center"><Button type="button" size="sm" variant="outline" onClick={enablePush}>{t('home.dayOfPush')}</Button>{pushStatus && <span className="text-xs" style={{ color: palette.fgMuted }}>{pushStatus}</span>}</div></div>
        <div className="grid gap-3 md:grid-cols-[10rem_1fr] items-center rounded-xl border p-3" style={{ borderColor: palette.border }}><div className="flex flex-col items-center gap-1"><QrCode className="h-4 w-4" /><StaffHelpQr payload={dayOf.qrPayload} /></div><div><strong>{t('home.dayOfStaffQr')}</strong><p className="mt-1 text-xs" style={{ color: palette.fgMuted }}>{t('home.dayOfStaffQrCopy')}</p><code className="mt-2 block break-all rounded bg-black/5 p-2 text-xs">{dayOf.qrPayload}</code></div></div>
...
function GuestReminderPreferences({ eventId, activeGuest, guestToken, palette, guestReminders }: { eventId: string; activeGuest?: PortalGuestEntry; guestToken: string; palette: Palette; guestReminders: PortalInfoResponse['guestReminders'] | null }) {
  ...
  const [language, setLanguage] = useState<string>(defaults.preferences.language || lang || 'en');
  ...
  const save = async (sendInfo?: 'schedule' | 'directions') => {
    if (!activeGuest?.id || !guestToken) return setStatus(t('home.remindersNeedLink'));
    try {
    ...
    } catch { setStatus(t('home.remindersFailed')); }
  };
  const options = [
  ...
  return <Card id="guest-reminder-preferences-info" style={{ background: palette.surface, borderColor: palette.border }} aria-label={t('home.remindersTitle')}><CardHeader><CardTitle className="font-display text-2xl flex items-center gap-2"><Bell className="h-5 w-5" /> {t('home.remindersTitle')}</CardTitle><CardDescription>{defaults.defaults.guestFriendlyCopy}</CardDescription></CardHeader><CardContent className="space-y-4 text-sm"><div className="grid gap-2 sm:grid-cols-2"><div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong>{t('home.remindersChannels')}</strong><p className="mt-1">{t('home.remindersEmailProvider', { state: defaults.providers.emailConnected ? t('common.connected') : t('common.notConnected') })}</p><p>{t('home.remindersSmsProvider', { state: defaults.providers.smsConnected ? t('common.connected') : t('common.notConnected') })}</p></div><div className="rounded-xl border p-3" style={{ borderColor: palette.border }}><strong className="flex items-center gap-2"><Clock className="h-4 w-4" /> {t('home.remindersQuietHours')}</strong><div className="mt-2 grid grid-cols-2 gap-2"><Input value={quietHoursStart} onChange={(e) => setQuietHoursStart(e.target.value)} aria-label={t('home.remindersQuietHoursStart')} placeholder="21:00" /><Input value={quietHoursEnd} onChange={(e) => setQuietHoursEnd(e.target.value)} aria-label={t('home.remindersQuietHoursEnd')} placeholder="08:00" /></div></div></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><label className="flex items-start gap-2 rounded-lg border p-2" style={{ borderColor: palette.border }}><input type="checkbox" checked={emailOptIn} onChange={(e) => setEmailOptIn(e.target.checked)} /> {t('home.remindersEmail')}</label><label className="flex items-start gap-2 rounded-lg border p-2" style={{ borderColor: palette.border }}><input type="checkbox" checked={smsOptIn} onChange={(e) => setSmsOptIn(e.target.checked)} /> {t('home.remindersSms')}</label><label className="text-sm">{t('home.remindersChannel')}<select value={confirmationPreference} onChange={(e) => setConfirmationPreference(e.target.value as any)} className="mt-1 h-10 w-full rounded-md border px-2" style={{ borderColor: palette.border, background: palette.surface }} aria-label={t('home.remindersChannel')}><option value="email">Email</option><option value="sms">SMS</option><option value="both">Both</option><option value="none">None</option></select></label><label className="text-sm">{t('home.remindersLanguage')}<select value={language} onChange={(e) => setLanguage(e.target.value)} className="mt-1 h-10 w-full rounded-md border px-2" style={{ borderColor: palette.border, background: palette.surface }} aria-label={t('home.remindersLanguage')}><option value="en">English</option><option value="es">Español</option><option value="fr">Français</option><option value="zh">中文</option></select></label></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{options.map(([type, label]) => <label key={type} className="flex items-start gap-2 rounded-lg border p-2" style={{ borderColor: palette.border }}><input type="checkbox" checked={reminderTypes.includes(type)} onChange={() => toggle(type)} /> {label}</label>)}</div><div className="flex flex-wrap gap-2 items-center"><Button type="button" onClick={() => save()}>{t('home.remindersSave')}</Button><Button type="button" variant="outline" onClick={() => save('schedule')}><Send className="h-4 w-4 mr-1" /> {t('home.remindersSendSchedule')}</Button><Button type="button" variant="outline" onClick={() => save('directions')}><Send className="h-4 w-4 mr-1" /> {t('home.remindersSendDirections')}</Button>{status && <span className="text-xs" style={{ color: palette.fgMuted }}>{status}</span>}</div></CardContent></Card>;
}
...
function GuestPrivacyConsent({ eventId, activeGuest, guestToken, palette, guestPrivacy }: { eventId: string; activeGuest?: PortalGuestEntry; guestToken: string; palette: Palette; guestPrivacy: PortalInfoResponse['guestPrivacy'] | null }) {
  ...
  return (
    <Card id="guest-privacy-consent-info" style={{ background: palette.surface, borderColor: palette.border }} aria-label={t('home.privacyTitle')}>
      <CardHeader><CardTitle className="font-display text-2xl flex items-center gap-2"><LockKeyhole className="h-5 w-5" /> {t('home.privacyTitle')}</CardTitle><CardDescription>{privacy.summary}</CardDescription></CardHeader>
      <CardContent className="space-y-4 text-sm">
        ...
        {privacy.correctionDeletion.enabled && <div className="rounded-xl border p-3 space-y-3" style={{ borderColor: palette.border, background: palette.accentSoft }}><strong className="flex items-center gap-2"><Trash2 className="h-4 w-4" /> {t('home.privacyDeleteTitle')}</strong><p className="text-xs" style={{ color: palette.fgMuted }}>Requests route to the {privacy.correctionDeletion.contactLabel}. This does not remove records the venue must retain for legal, accounting, safety, or event operations.</p><div className="grid gap-2 sm:grid-cols-3"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" /><Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email for follow-up" /><select className="h-10 rounded-md border px-2 text-sm" style={{ borderColor: palette.border, background: palette.surface }} value={requestType} onChange={(e) => setRequestType(e.target.value as typeof requestType)} aria-label="Privacy request type"><option value="update_contact">Update contact info</option><option value="delete_contact">Delete contact info</option><option value="consent_change">Change reminder consent</option><option value="data_question">Data/privacy question</option></select></div><textarea className="min-h-20 w-full rounded-md border p-2 text-sm" style={{ borderColor: palette.border, background: palette.surface }} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Tell us what contact info should be updated/deleted or what privacy question you have." aria-label="Privacy request message" /><div className="flex flex-wrap gap-2 items-center"><Button type="button" onClick={send}>{t('home.privacySend')}</Button>{privacy.correctionDeletion.contactEmail && <a className="text-xs font-bold underline" href={`mailto:${privacy.correctionDeletion.contactEmail}`}>{t('home.privacyEmailContact')}</a>}{status && <span className="text-xs" style={{ color: palette.fgMuted }}>{status}</span>}</div></div>}
      </CardContent>
    </Card>
  );
}
...
function GuestAccessibilityCareCenter({ eventId, activeGuest, guestToken, palette, guestCare, guestTravel }: { eventId: string; activeGuest?: PortalGuestEntry; guestToken: string; palette: Palette; guestCare: PortalInfoResponse['guestCare'] | null; guestTravel: PortalInfoResponse['guestTravel'] | null }) {
  const { lang, t } = useI18n();
  ...
  return (
    <Card id="guest-accessibility-care-info" style={{ background: palette.surface, borderColor: palette.border }} aria-label="Guest Accessibility and Care Center">
      <CardHeader>
        <CardTitle className="font-display text-2xl flex items-center gap-2"><Accessibility className="h-5 w-5" /> {t('home.careTitle')}</CardTitle>
      ...
      <CardContent className="space-y-5 text-sm">
        ...
        <div className="rounded-xl border p-3 space-y-3" style={{ borderColor: palette.border }}>
          ...
          <div className="grid gap-2 md:grid-cols-2">
            ...
            <CareTextarea icon={<Languages className="h-4 w-4" />} label={t('home.careInterpreter')} value={interpretationLanguage} setValue={setInterpretationLanguage} placeholder={`${t('home.careInterpreterPlaceholder')}: ${lang}`} />
          ...
          <div className="flex flex-wrap items-center gap-2"><Label className="text-xs font-bold uppercase">{t('home.careFollowUp')}</Label><select value={contactPreference} onChange={(e) => setContactPreference(e.target.value as typeof contactPreference)} className="h-9 rounded-md border px-2 text-sm" style={{ borderColor: palette.border, background: palette.surface }} aria-label="Accessibility request contact preference"><option value="email">Email</option><option value="phone">Phone</option><option value="text">Text</option><option value="in_app">Portal message</option></select><Button type="button" onClick={submit}>{t('home.careSend')}</Button>{status && <span className="text-xs" style={{ color: palette.fgMuted }}>{status}</span>}</div>
        </div>
      </CardContent>
    </Card>
  );
...
function GuestFaqEtiquette({ eventId, activeGuest, guestToken, palette, branding, guestFaq, config }: { eventId: string; activeGuest?: PortalGuestEntry; guestToken: string; palette: Palette; branding: PortalInfoResponse['branding'] | null; guestFaq: PortalInfoResponse['guestFaq'] | null; config: Record<string, any> }) {
  const { lang, t } = useI18n();
  ...
  const [language, setLanguage] = useState<string>(lang || fallbackFaq.multilingual.availableLanguages[0]?.code || 'en');
  ...
  return (
    <Card id="guest-faq-info" style={{ background: palette.surface, borderColor: palette.border }}>
      <CardHeader>
        <CardTitle className="font-display text-2xl flex items-center gap-2"><HelpCircle className="h-5 w-5" /> {t('home.faqTitle')}</CardTitle>
      ...
      <CardContent className="space-y-5 text-sm">
        ...
        {fallbackFaq.askQuestion.enabled && <div className="rounded-xl border p-3 space-y-3" style={{ borderColor: palette.border, background: palette.accentSoft }}>
          ...
          <div className="flex flex-wrap items-center gap-2"><Button type="button" onClick={submitQuestion}>{t('home.faqAsk')}</Button>{branding?.supportEmail && <a className="text-xs underline font-bold" href={`mailto:${branding.supportEmail}`}>{t('home.faqEmailInstead')}</a>}{questionStatus && <span className="text-xs" style={{ color: palette.fgMuted }}>{questionStatus}</span>}</div>
        </div>}
      </CardContent>
    </Card>
  );
...
function GuestRegistryGifts({ config, palette, guestGifts }: { config: Record<string, any>; palette: Palette; guestGifts: PortalInfoResponse['guestGifts'] | null }) {
  ...
  return (
    <Card id="guest-registry-gifts-info" style={{ background: palette.surface, borderColor: palette.border }}>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2"><Gift className="h-4 w-4" /> {t('home.giftsTitle')}</CardTitle>
