...
type CheckInStatus = 'expected' | 'arrived' | 'setup' | 'completed' | 'departed';

// Simulated local state for check-ins since we don't have a check-in table explicitly defined
export function VendorCheckInApp({ eventId, organizationId }: Props) {
  ...
  const updateStatus = (vendorId: string, status: CheckInStatus) => {
    setStatusLog(prev => ({ ...prev, [vendorId]: status }));
  };

  const filteredVendors = vendors.filter(v => {
    const s = statusLog[v.id] || 'expected';
    if (filter === 'expected' && s !== 'expected') return false;
    if (filter === 'arrived' && (s === 'expected' || s === 'departed')) return false;
    
    // Naive late simulation
    if (filter === 'late' && s !== 'expected') return false;

    if (search && !v.name.toLowerCase().includes(search.toLowerCase()) && !v.category?.toLowerCase().includes(search.toLowerCase())) {
      return false;
    }
    return true;
  });
  ...
  const expectedCount = vendors.filter(v => (statusLog[v.id] || 'expected') === 'expected').length;
  const arrivedCount = vendors.filter(v => ['arrived', 'setup', 'completed'].includes(statusLog[v.id])).length;
  ...
  return (
    <div className="min-h-screen bg-surface-2/50 pb-20">
       <header className="bg-surface border-b border-border sticky top-0 z-20">
          ...
          <div className="max-w-4xl mx-auto px-4 py-3 border-t border-border/50 flex gap-2 overflow-x-auto no-scrollbar">
             <button onClick={() => setFilter('all')} className={cn("px-4 py-1.5 text-sm font-medium rounded-full transition-colors whitespace-nowrap", filter === 'all' ? "bg-fg text-surface" : "bg-surface-2 text-fg-muted hover:text-fg")}>
                All Vendors <span className="ml-1 opacity-70">({vendors.length})</span>
             </button>
             <button onClick={() => setFilter('expected')} className={cn("px-4 py-1.5 text-sm font-medium rounded-full transition-colors whitespace-nowrap", filter === 'expected' ? "bg-fg text-surface" : "bg-surface-2 text-fg-muted hover:text-fg")}>
                Expected <span className="ml-1 opacity-70">({expectedCount})</span>
             </button>
             <button onClick={() => setFilter('arrived')} className={cn("px-4 py-1.5 text-sm font-medium rounded-full transition-colors whitespace-nowrap", filter === 'arrived' ? "bg-fg text-surface" : "bg-surface-2 text-fg-muted hover:text-fg")}>
                On-Site <span className="ml-1 opacity-70">({arrivedCount})</span>
             </button>
             <button onClick={() => setFilter('late')} className={cn("px-4 py-1.5 text-sm font-medium rounded-full transition-colors whitespace-nowrap flex items-center gap-1", filter === 'late' ? "bg-danger text-danger-fg" : "bg-danger/10 text-danger hover:bg-danger/20")}>
                <AlertCircle className="w-3.5 h-3.5" /> Late <span className="ml-1 opacity-70">(0)</span>
             </button>
          </div>
       </header>
          ...
          <div className="flex gap-2 mb-6">
             <div className="flex-1 relative">
                ...
                <Input 
                  ...
                  className="pl-10 h-12 text-lg rounded-xl bg-surface shadow-sm border-border"
                  value={search}
          ...
          {filteredVendors.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {filteredVendors.map(vendor => {
                  const status = statusLog[vendor.id] || 'expected';
                        ...
                        <div className={cn("p-4", status !== 'expected' ? "bg-brand/5" : "bg-surface")}>
                           <div className="flex justify-between items-start mb-3">
                              <div>
                                 ...
                                 <div className="text-sm font-medium text-fg-subtle uppercase tracking-widest mt-1">
                                    {vendor.category}
                                 </div>
                              </div>
                              <Badge variant={
                                 status === 'expected' ? 'outline' : 
                                 status === 'departed' ? 'info' : 'success'
                           ...
                           <div className="space-y-1 mb-6 text-sm">
                              {vendor.contact_name && (
                                <div className="flex items-center gap-2 text-fg-muted">
                                   <UserCircle className="w-4 h-4 opacity-50" /> {vendor.contact_name}
                                </div>
                              )}
                              {vendor.phone && (
                                <div className="flex items-center gap-2 text-fg-muted">
                                   <Phone className="w-4 h-4 opacity-50" /> {vendor.phone}
                                </div>
                              )}
                           </div>
                           ...
                           <div className="flex flex-wrap gap-2 pt-4 border-t border-border/50">
                              ...
                              {(status === 'completed' || status === 'arrived' || status === 'setup') && (
