...
function buildTransporter(ctx: IntegrationContext): Transporter {
...
function fromHeader(ctx: IntegrationContext): string {
  const cfg = configSchema.parse(ctx.config);
  return cfg.fromName ? `"${cfg.fromName}" <${cfg.fromAddress}>` : cfg.fromAddress;
}

const sendEmail: ProviderAction<SendEmailInput, SendEmailResult> = {
  id: 'sendEmail',
  label: 'Send email',
...
export const emailSmtpProvider: IntegrationProvider = {
  id: 'email_smtp',
  name: 'Email (SMTP)',
  category: 'email',
  description:
    'Send transactional email — RSVP confirmations, magic-link guest invitations, ' +
  ...
  async verify(ctx) {
    ...
    try {
      ...
      if (!ok) throw new Error('SMTP server did not accept the credentials.');
    } catch (e) {
