...
export type PermissionId =
  ...
  // ─── Roles (the new RBAC admin surface) ───────────────
  | 'roles.view'
  | 'roles.manage'
...
export interface PermissionDefinition {
  ...
  category: 'organization' | 'roles' | 'events' | 'venues' | 'layouts'
          ...
          | 'vendor_portal' | 'timeline' | 'staff' | 'questions'
          | 'messages' | 'audit';
}
...
// ─── System roles ─────────────────────────────────────
// These are seeded into the `roles` table at app startup. The ids are
// stable strings (NOT uuids) so route code, tests, and seeds can reference
// them by id directly. Once seeded, system rows are flagged is_system=1
// and the route layer refuses to edit/delete them.
