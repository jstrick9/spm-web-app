 ...
 *
 *   GET    /api/orgs/:orgId/integrations              list this org's integrations
 *   GET    /api/orgs/:orgId/integrations/providers    list AVAILABLE providers
 *   POST   /api/orgs/:orgId/integrations              create / update credentials (api_key / smtp kinds)
 *   POST   /api/orgs/:orgId/integrations/:provider/test  re-run verify()
 *   PATCH  /api/integrations/:id                       update display name / disable / enable
 *   DELETE /api/integrations/:id                       remove the integration
 *   GET    /api/integrations/:id/events                list audit events
 *
 *   (OAuth start/callback + webhook receiver are in separate route files.)
 *
 * Secrets are NEVER returned in responses. The `secret_payload` column
 * is opaque; we only return `hasSecrets: boolean`.
 */
...
function publicView(row: ReturnType<typeof integrationsRepo.findById>) {
  if (!row) return null;
...
export async function integrationRoutes(app: FastifyInstance) {
  ...
  app.get(
    '/api/orgs/:orgId/integrations/providers',
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!provDef) throw BadRequest('unknown-provider', { provider });
      ...
      try {
        ...
        auditRepo.log({
          ...
          ip: req.ip, details: { provider, error: (e as Error).message },
        });
  ...
  // ─── Test connection ───────────────────────────────────
  app.post(
    '/api/orgs/:orgId/integrations/:provider/test',
    { preHandler: requireAuth },
    async (req, reply) => {
