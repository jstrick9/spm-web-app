...
async function registerUser(email = `u-${Math.random().toString(36).slice(2)}@x.com`) {
  ...
  const body = res.json();
  return { token: body.token as string, userId: body.user.id as string, orgId: body.organizationId as string, email };
}

async function authedRequest(token: string, method: 'GET'|'POST'|'PUT'|'PATCH'|'DELETE', url: string, payload?: unknown) {
  return app.inject({
    method, url,
