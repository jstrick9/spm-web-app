...
export async function layoutRoutes(app) {
    app.get('/api/orgs/:orgId/layouts', { preHandler: requireAuth }, async (req) => {
        ...
        const { eventId, template } = req.query;
        if (!can(req.auth.memberships, { organizationId: orgId }, 'layouts.view'))
            throw Forbidden();
        return {
    ...
    app.post('/api/layouts', { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!can(req.auth.memberships, { organizationId: parsed.data.organizationId }, 'layouts.create'))
            throw Forbidden();
    ...
    app.get('/api/layouts/:id', { preHandler: requireAuth }, async (req) => {
        ...
        if (!can(req.auth.memberships, { organizationId: layout.organization_id }, 'layouts.view', orgMap))
            throw Forbidden();
        return { layout };
    });
    app.post('/api/layouts/:id/save', { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!layout)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: layout.organization_id }, 'layouts.edit'))
            throw Forbidden();
    ...
    app.get('/api/layouts/:id/versions', { preHandler: requireAuth }, async (req) => {
        ...
        if (!layout)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: layout.organization_id }, 'layouts.view'))
            throw Forbidden();
    ...
    app.delete('/api/layouts/:id', { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!layout)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: layout.organization_id }, 'layouts.delete'))
            throw Forbidden();
