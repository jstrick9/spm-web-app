...
export async function guestRoutes(app) {
    ...
    app.patch('/api/guests/:id', { preHandler: requireAuth }, async (req) => {
        ...
        if (!guest)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: guest.organization_id }, 'guests.manage'))
            throw Forbidden();
    ...
    app.get('/api/events/:eventId/portal-config', { preHandler: requireAuth }, async (req) => {
        const { eventId } = req.params;
        const orgMap = eventsRepo.orgMapForUser(req.auth.userId);
        if (!can(req.auth.memberships, { eventId }, 'portal.config.manage', orgMap))
            throw Forbidden();
    ...
    app.put('/api/events/:eventId/portal-config', { preHandler: requireAuth }, async (req) => {
        ...
        if (!event)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: event.organization_id }, 'portal.config.manage'))
            throw Forbidden();
    ...
    app.get('/api/portal/:eventId/info', async (req, reply) => {
        ...
        if (!event)
            throw NotFound();
        const cfg = portalConfigRepo.getForEvent(eventId);
    ...
    app.post('/api/portal/:eventId/verify-password', async (req, reply) => {
        ...
        const cfg = portalConfigRepo.getForEvent(eventId);
        if (!cfg?.password_hash || !cfg.password_salt)
    ...
    app.post('/api/portal/:eventId/rsvp', async (req, reply) => {
        ...
        if (parsed.data.guestId) {
            ...
            if (!g || g.event_id !== eventId)
                throw BadRequest('guest-not-in-event');
            if (!g.allow_portal_access)
                throw new (await import('../lib/errors.js')).HttpError(403, 'portal-access-revoked');
        }
