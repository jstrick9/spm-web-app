...
import { auditRepo, eventsRepo, orgsRepo, subEventsRepo, } from '../db/repos/index.js';
...
export async function eventRoutes(app) {
    ...
    app.put('/api/orgs/:orgId/branding', { preHandler: requireAuth }, async (req, reply) => {
        ...
        orgsRepo.updateBranding(orgId, req.body ?? {});
        auditRepo.log({
    ...
    app.patch('/api/events/:eventId', { preHandler: requireAuth }, async (req) => {
        ...
        if (!event)
            throw NotFound();
        const orgMap = eventsRepo.orgMapForUser(req.auth.userId);
        if (!can(req.auth.memberships, { eventId }, 'events.edit', orgMap))
            throw Forbidden();
        ...
        for (const [k, col] of Object.entries(keyMap)) {
            if (k in dataIn && dataIn[k] !== undefined)
                patch[col] = dataIn[k];
        }
        const updated = eventsRepo.update(eventId, patch);
        auditRepo.log({
    ...
    app.delete('/api/events/:eventId', { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!event)
            throw NotFound();
        const orgMap = eventsRepo.orgMapForUser(req.auth.userId);
        if (!can(req.auth.memberships, { eventId }, 'events.delete', orgMap))
            throw Forbidden();
