...
import { staffTasksRepo, staffAreasRepo, staffShiftsRepo, } from '../db/repos/index.js';
...
export async function staffRoutes(app) {
    ...
    app.post('/api/orgs/:orgId/staff/tasks', { preHandler: requireAuth }, async (req, reply) => {
        const { orgId } = req.params;
        if (!can(req.auth.memberships, { organizationId: orgId }, 'staff.manage'))
            throw Forbidden();
    ...
    app.patch('/api/staff/tasks/:id', { preHandler: requireAuth }, async (req) => {
        ...
        if (!task)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: task.organization_id }, 'staff.manage'))
            throw Forbidden();
    ...
    app.delete('/api/staff/areas/:id', { preHandler: requireAuth }, async (req, reply) => {
        staffAreasRepo.delete(req.params.id);
        return reply.code(204).send();
    });
    // ─── Shifts ────────────────────────────────────────────
    ...
    app.delete('/api/staff/shifts/:id', { preHandler: requireAuth }, async (req, reply) => {
        staffShiftsRepo.delete(req.params.id);
        return reply.code(204).send();
    });
}
