...
async function register(email = `cov-${Math.random()}@x.com`) {
    ...
    return { token: r.json().token, orgId: r.json().organizationId, userId: r.json().user.id };
}
const req = (token, method, url, payload) => app.inject({
    method, url,
...
//# sourceMappingURL=coverage.integration.test.js.map