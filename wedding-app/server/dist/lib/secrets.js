...
function parseKey(raw) {
    ...
    throw new Error(`WEDDING_SECRETS_KEY must be 32 bytes (64 hex chars or 44 base64 chars). ` +
...
export function getMasterKey() {
    ...
    if (!raw) {
        if (process.env.NODE_ENV === 'test') {
            ...
            throw new Error('WEDDING_SECRETS_KEY is not set. Generate one with `openssl rand -hex 32` ' +
...
export function _resetMasterKey() {
    cachedKey = null;
}
/**
 ...
 */
export function canDecrypt(sealed) {
    try {
        openSecret(sealed);
        return true;
    }
