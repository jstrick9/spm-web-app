...
import { catalogRepo, auditRepo } from '../db/repos/index.js';
...
export async function catalogRoutes(app) {
    app.get('/api/orgs/:orgId/catalog/:kind', { preHandler: requireAuth }, async (req) => {
        const { orgId, kind } = req.params;
        if (!KIND_RE.test(kind))
            throw BadRequest('invalid-kind');
        if (!can(req.auth.memberships, { organizationId: orgId }, 'catalog.view'))
            throw Forbidden();
        return { items: catalogRepo.listForOrg(orgId, kind) };
    });
    app.post('/api/orgs/:orgId/catalog/:kind', { preHandler: requireAuth }, async (req, reply) => {
        const { orgId, kind } = req.params;
        if (!KIND_RE.test(kind))
            throw BadRequest('invalid-kind');
        if (!can(req.auth.memberships, { organizationId: orgId }, 'catalog.manage'))
            throw Forbidden();
        const parsed = itemSchema.safeParse(req.body);
        if (!parsed.success)
            throw BadRequest('invalid-input', parsed.error.issues);
        const item = catalogRepo.create(orgId, { kind: kind, ...parsed.data });
        auditRepo.log({
    ...
    app.put('/api/orgs/:orgId/catalog/:kind', { preHandler: requireAuth }, async (req) => {
        ...
        if (!parsed.success)
            throw BadRequest('invalid-input', parsed.error.issues);
        const items = catalogRepo.replaceAll(orgId, kind, parsed.data.items);
        auditRepo.log({
            organizationId: orgId, actorUserId: req.auth.userId, actorLabel: req.auth.email,
    ...
    app.patch('/api/catalog/:id', { preHandler: requireAuth }, async (req) => {
        ...
        if (!item)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: item.organizationId }, 'catalog.manage'))
            throw Forbidden();
