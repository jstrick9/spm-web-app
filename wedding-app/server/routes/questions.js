...
export async function questionRoutes(app) {
    app.get('/api/orgs/:orgId/questions', { preHandler: requireAuth }, async (req) => {
        ...
        if (!can(req.auth.memberships, { organizationId: orgId }, 'questions.view'))
            throw Forbidden();
    ...
    app.post('/api/orgs/:orgId/questions', { preHandler: requireAuth }, async (req, reply) => {
        const { orgId } = req.params;
        if (!can(req.auth.memberships, { organizationId: orgId }, 'questions.manage'))
            throw Forbidden();
    ...
    app.delete('/api/questions/:id', { preHandler: requireAuth }, async (req, reply) => {
        eventQuestionsRepo.delete(req.params.id);
