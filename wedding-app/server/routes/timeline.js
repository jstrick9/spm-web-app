...
export async function timelineRoutes(app) {
    ...
    app.post('/api/events/:eventId/timeline', { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!event)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: event.organization_id }, 'timeline.manage'))
            throw Forbidden();
    ...
    app.patch('/api/timeline/:id', { preHandler: requireAuth }, async (req) => {
        ...
        if (!item)
            throw NotFound();
        if (!can(req.auth.memberships, { organizationId: item.organization_id }, 'timeline.manage'))
            throw Forbidden();
