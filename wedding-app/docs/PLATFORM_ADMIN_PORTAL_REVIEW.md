...
| High | Team management | The admin UI used the legacy registered-user-only add-member endpoint even though the backend supports invitations for unregistered users. | Team onboarding dead end and inaccurate admin guidance. | **Fixed**: Team Members now uses the invitation lifecycle and correctly explains immediate-add vs. invitation behavior. Role hierarchy/pending-invite review continues. |
