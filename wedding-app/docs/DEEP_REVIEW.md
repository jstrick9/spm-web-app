...
| F-6 | Decomposed `routes/couple.ts` (2,190 → 22 lines) into `routes/couple/{planning,guests,finance,portal,documents,postEvent,shared}`, `routes/guests.ts` (1,589 → 14 lines) into `routes/guests/{core,portal,shared}`; **client follow-up**: `CatalogScreen` (2,962 → 169 + `managers/*`), `DashboardScreen` (2,513 → 861 + `dashboardPanels`), `CanvasPage` (2,874 → 2,535 + `canvasPanels`), `EventDetail` (1,973 → 637 + `eventDetailPanels`), `AdminPanel` (1,123 → 470 + `adminPanels`), `EventVendorsTab` (1,348 → 945 + `vendorPanels`), `EventStaffTab` (1,389 → 1,235 + `staffPanels`), `GuestPortalSettingsTab` (1,697 → 1,465 + `portalSettingsPanels`), `CanvasPage` (2,874 → 2,535 + `canvasPanels`; then sections pass → 1,227 + `canvasSections/{CanvasToolbar,CanvasSidebar,CanvasStageArea}` with typed prop contracts) | all 44 couple + 88 guest integration tests green; full client suite 752/752 green (14 new direct CanvasPage-section tests); typechecks + production build + bundle budgets pass |
